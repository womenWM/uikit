...
export default (props: ViewerProps) => {
  ...
  }, [props.container, setContainerWidthHeight]);
  ...
  }, [props.container, state.visible, visible]);
  ...
  }, [state.startLoading, state.activeIndex, loadImg]);
  ...
  function handleRotate(isRight: boolean = false) {
    const rotate = state.rotate + 90 * (isRight ? 1 : -1);
    dispatch(createAction(ACTION_TYPES.update, {
      rotate,
    }));
  ...
  return (
    <div
      ...
      ref={viewerCore}
    >
      <div className={`${prefixCls}-mask`} style={{ zIndex, display: 'none' }} />
      {props.noClose || (
        <div
